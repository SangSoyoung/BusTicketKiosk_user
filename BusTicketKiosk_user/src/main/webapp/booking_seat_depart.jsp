...
<h3>좌석 선택</h3>
<form action = "timetable_arrv.do" method="get" onsubmit="return doCheck();">
<div id="seat">
<c:forEach var="se" items="${seatlist }">
	<c:if test="${se.booking eq 0}">
		<input type="checkbox" name="go_seatArr" value="${se.seatNo }" onclick="checkDisabled(this, checkLimit(this), ${pnum });">
	</c:if>
	<c:if test="${se.booking eq 1}">
		<input type="checkbox" name="go_seatArr" value="${se.seatNo }" onclick="return false" checked disabled>
	</c:if>															
</c:forEach>
</div>
...
<script>
	...
	// 정해진 개수까지 체크해야함
	function doCheck(){
		var chk = document.getElementsByName("go_seatArr");
